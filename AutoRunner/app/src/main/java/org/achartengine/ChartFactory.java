...
public class ChartFactory {
     ...
     *             or if the dataset and the renderer don't include the same
     *             number of series
     */
     ...
     * @param dataset the multiple series dataset (cannot be null)
     * @param renderer the multiple series renderer (cannot be null)
     * @param format the date format pattern to be used for displaying the X
     *            axis date labels. If null, a default appropriate format will
     *            be used.
    ...
    public static final GraphicalView getTimeChartView(Context context,
        ...
        chart.setDateFormat(format, locale);
        return new GraphicalView(context, chart);
    }
     ...
     * @throws IllegalArgumentException if dataset is null or renderer is null
     *             or if a dataset number of items is different than the number
     ...
     * @throws IllegalArgumentException if dataset is null or renderer is null
     *             or if the dataset number of items is different than the
     *             number of series renderers
     */
    ...
    public static final Intent getTimeChartIntent(Context context, XYMultipleSeriesDataset dataset,
            XYMultipleSeriesRenderer renderer, String format, Locale locale) {
        return getTimeChartIntent(context, dataset, renderer, format, "", locale);
    }
     ...
     *            then the title bar will be hidden. If a blank title is passed
     *            in, then the title bar will be the default. Pass in any other
     *            string to set a custom title.
     * @return a line chart intent
     * @throws IllegalArgumentException if dataset is null or renderer is null
     *             or if the dataset and the renderer don't include the same
     *             number of series
     ...
     */
    public static final Intent getTimeChartIntent(Context context, XYMultipleSeriesDataset dataset,
            XYMultipleSeriesRenderer renderer, String format, String activityTitle, Locale locale) {
        checkParameters(dataset, renderer);
        Intent intent = new Intent(context, GraphicalActivity.class);
        TimeChart chart = new TimeChart(dataset, renderer);
        chart.setDateFormat(format, locale);
    ...
    public static final Intent getCombinedXYChartIntent(Context context,
            XYMultipleSeriesDataset dataset, XYMultipleSeriesRenderer renderer,
            XYCombinedChartDef[] types, String activityTitle) {
        checkParameters(dataset, renderer);
        Intent intent = new Intent(context, GraphicalActivity.class);
        CombinedXYChart chart = new CombinedXYChart(dataset, renderer, types);
        intent.putExtra(CHART, chart);
