...
public class BarChart extends XYChart {
    ...
    /**
     * The previous series Y axis point limits to be used for HEAP type bar
     * charts.
     */
    ...
    protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values,
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (mType == Type.STACKED || mType == Type.HEAPED) {
                ret[i / 2] = new ClickableArea(new RectF(x - halfDiffX, Math.min(y, yAxisValue),
                        x + halfDiffX, Math.max(y, yAxisValue)), values.get(i), values.get(i + 1));
            }
            else {
                float startX = x - seriesNr * halfDiffX + seriesIndex * 2 * halfDiffX;
                ret[i / 2] = new ClickableArea(new RectF(startX, Math.min(y, yAxisValue),
                        startX + 2 * halfDiffX, Math.max(y, yAxisValue)), values.get(i),
                        values.get(i + 1));
            }
        }
        return ret;
    ...
    protected void drawBar(Canvas canvas, float xMin, float yMin, float xMax, float yMax, int scale,
        ...
        if (renderer.isGradientEnabled()) {
            ...
            if (yMin < minY) {
                ...
                canvas.drawRect(Math.round(xMin),
                        Math.round(yMin),
                        Math.round(xMax),
                        Math.round(gradientMinY),
                        paint);
            }
            else {
            ...
            GradientDrawable gradient = new GradientDrawable(Orientation.BOTTOM_TOP,
                    new int[] { gradientStartColor, gradientStopColor });
            gradient.setBounds(Math.round(xMin),
                    Math.round(gradientMinY),
                    Math.round(xMax),
                    Math.round(gradientMaxY));
    ...
    protected int getGradientPartialColor(int minColor, int maxColor, float fraction) {
        int alpha = Math
                .round(fraction * Color.alpha(minColor) + (1 - fraction) * Color.alpha(maxColor));
        int r = Math.round(fraction * Color.red(minColor) + (1 - fraction) * Color.red(maxColor));
        int g = Math
                .round(fraction * Color.green(minColor) + (1 - fraction) * Color.green(maxColor));
        int b = Math
                .round(fraction * Color.blue(minColor) + (1 - fraction) * Color.blue((maxColor)));
