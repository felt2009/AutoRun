...
public abstract class AbstractChart implements Serializable {
     ...
     */
    protected boolean getExceed(float currentWidth, DefaultRenderer renderer, int right,
            int width) {
    ...
    protected void drawPath(Canvas canvas, List<Float> points, Paint paint, boolean circular) {
        ...
        tempDrawPoints = calculateDrawPoints(points.get(0),
                points.get(1),
                points.get(2),
                points.get(3),
                height,
    ...
    protected void drawPath(Canvas canvas, float[] points, Paint paint, boolean circular) {
        ...
        tempDrawPoints = calculateDrawPoints(points[0],
                points[1],
                points[2],
                points[3],
                height,
        ...
        for (int i = 4; i < length; i += 2) {
            ...
            tempDrawPoints = calculateDrawPoints(points[i - 2],
                    points[i - 1],
                    points[i],
                    points[i + 1],
                    height,
    ...
    protected void drawLabel(Canvas canvas, String labelText, DefaultRenderer renderer,
            List<RectF> prevLabelsBounds, int centerX, int centerY, float shortRadius,
            float longRadius, float currentAngle, float angle, int left, int right, int color,
        ...
        if (renderer.isShowLabels() || display) {
            ...
            while (!okBounds && line) {
                ...
                for (int j = 0; j < length && !intersects; j++) {
                    ...
                    if (prevLabelBounds.intersects(xLabel,
                            yLabel,
                            xLabel + widthLabel,
                            yLabel + size)) {
                        intersects = true;
