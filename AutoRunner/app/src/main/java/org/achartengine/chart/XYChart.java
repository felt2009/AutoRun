...
public abstract class XYChart extends AbstractChart {
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        if (paint.getTypeface() == null
                || (mRenderer.getTextTypeface() != null
                        && paint.getTypeface().equals(mRenderer.getTextTypeface()))
            ...
            synchronized (series) {
                ...
                for (Entry<Double, Double> value : range.entrySet()) {
                    ...
                    if (!isNullValue(yValue)) {
                        ...
                        points.add(
                                (float) (bottom - yPixelsPerUnit[scale] * (yValue - minY[scale])));
                    }
                ...
                if (count > 0) {
                    ...
                    for (int j = 0; j < count; j++) {
                        float xS = (float) (left
                                + xPixelsPerUnit[scale] * (series.getAnnotationX(j) - minX[scale]));
                        float yS = (float) (bottom
                                - yPixelsPerUnit[scale] * (series.getAnnotationY(j) - minY[scale]));
                        paint.getTextBounds(series.getAnnotationAt(j),
                                0,
                                series.getAnnotationAt(j).length(),
                                bound);
        ...
        if (showLabels || showGridX) {
            List<Double> xLabels = getValidLabels(
                    getXLabels(minX[0], maxX[0], mRenderer.getXLabels()));
                ...
                for (int i = 0; i < maxScaleNumber; i++) {
                    ...
                    for (Double location : yTextLabelLocations) {
                        if (minY[i] <= location && location <= maxY[i]) {
                            float yLabel = (float) (bottom
                                    - yPixelsPerUnit[i] * (location.doubleValue() - minY[i]));
                                ...
                                if (axisAlign == Align.LEFT) {
                                    ...
                                    if (showTickMarks) {
                                        canvas.drawLine(right,
                                                yLabel,
                                                right + getLabelLinePos(axisAlign),
                                                yLabel,
                                                paint);
                                    }
                                    drawText(canvas,
            ...
            if (showLabels) {
                ...
                if (or == Orientation.HORIZONTAL) {
                    ...
                    drawText(canvas,
                            mRenderer.getXTitle(),
                            x + width / 2,
                            y + height - size + mRenderer.getXLabelsPadding(),
                            paint,
                            -90);
        ...
        if (or == Orientation.HORIZONTAL) {
            drawLegend(canvas,
                    ...
                    left,
                    right,
                    y + (int) mRenderer.getXLabelsPadding(),
                    width,
                    height,
                    legendSize,
                    paint,
    ...
    protected Map<Integer, List<Double>> getYLabels(double[] minY, double[] maxY,
            int maxScaleNumber) {
                ...
                if (k == 2) { // decide whether to display first two points' values or
                    ...
                    if (Math.abs(points.get(2) - points.get(0)) > renderer
                        ...
                        drawText(canvas,
                                ...
                                points.get(0),
                                points.get(1) - renderer.getChartValuesSpacing(),
                                paint,
                                0);
                        // second point
                        drawText(canvas,
                                ...
                                points.get(2),
                                points.get(3) - renderer.getChartValuesSpacing(),
                                paint,
                                0);
     ...
     * @param xPixelsPerUnit the amount of pixels per one unit in the chart
     *            labels
    ...
    protected void drawXLabels(List<Double> xLabels, Double[] xTextLabelLocations, Canvas canvas,
            ...
            if (showLabels) {
                ...
                if (showTickMarks) {
                    canvas.drawLine(xLabel,
                            bottom,
                            xLabel,
                            bottom + mRenderer.getLabelsTextSize() / 3,
                            paint);
                }
                drawText(canvas,
                        getLabel(mRenderer.getXLabelFormat(), label),
                        xLabel,
                        bottom + mRenderer.getLabelsTextSize() * 4 / 3
        ...
        drawXTextLabels(xTextLabelLocations,
                canvas,
                paint,
                showLabels,
                left,
     ...
     * @param bottom the bottom value of the labels area
     * @param yPixelsPerUnit the amount of pixels per one unit in the chart
     *            labels
    ...
    protected void drawYLabels(Map<Integer, List<Double>> allYLabels, Canvas canvas, Paint paint,
            int maxScaleNumber, int left, int right, int bottom, double[] yPixelsPerUnit,
            double[] minY) {
        Orientation or = mRenderer.getOrientation();
        boolean showGridX = mRenderer.isShowGridX();
        boolean showLabels = mRenderer.isShowLabels();
                ...
                if (or == Orientation.HORIZONTAL) {
                    if (showLabels && !textLabel) {
                        ...
                        if (axisAlign == Align.LEFT) {
                            ...
                            drawText(canvas,
                                    getLabel(mRenderer.getYLabelFormat(i), label),
                                    left - mRenderer.getYLabelsPadding(),
                                    yLabel - mRenderer.getYLabelsVerticalPadding(),
                                    paint,
                                    mRenderer.getYLabelsAngle());
        ...
        if (showLabels) {
            ...
            for (Double location : xTextLabelLocations) {
                if (minX <= location && location <= maxX) {
                    float xLabel = (float) (left
                            + xPixelsPerUnit * (location.doubleValue() - minX));
                    paint.setColor(mRenderer.getXLabelsColor());
                    if (showTickMarks) {
                        canvas.drawLine(xLabel,
                                bottom,
    ...
    public double[] toRealPoint(float screenX, float screenY, int scale) {
        ...
        if (!mRenderer.isMinXSet(scale) || !mRenderer.isMaxXSet(scale)
                || !mRenderer.isMinYSet(scale) || !mRenderer.isMaxYSet(scale)) {
        ...
        if (mScreenR != null) {
            return new double[] {
                    ...
                    (mScreenR.top + mScreenR.height() - screenY) * (realMaxY - realMinY)
    ...
    public double[] toScreenPoint(double[] realPoint, int scale) {
        ...
        if (mScreenR != null) {
            return new double[] {
                    (realPoint[0] - realMinX) * mScreenR.width() / (realMaxX - realMinX)
                            + mScreenR.left,
                    (realMaxY - realPoint[1]) * mScreenR.height() / (realMaxY - realMinY)
                            + mScreenR.top };
        }
        else {
            return realPoint;
                ...
                if (clickableAreas.get(seriesIndex) != null) {
                    ...
                    for (ClickableArea area : clickableAreas.get(seriesIndex)) {
                        if (area != null) {
                            ...
                            if (rectangle != null
                                    && rectangle.contains(screenPoint.getX(), screenPoint.getY())) {
